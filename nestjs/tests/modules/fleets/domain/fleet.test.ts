  ...
  describe('Invariants', () => {
    ...
    describe('operation region validation', () => {
      it('should throw if operation region is invalid', () => {
        fc.assert(
          fc.property(fc.string().filter(s => !(s in FleetOperationRegion)), (invalidRegion) => {
            const builder = FleetBuilder.aFleet().withOperationRegion(invalidRegion as FleetOperationRegion)
